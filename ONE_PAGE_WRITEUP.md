...
- 67 tests covering fail-closed screening, margin rejection, aggregate collateral, account-level interest exposure, the position cap, DTE/OTM/spread filters, and submitted-vs-filled reconciliation.
...
**Adversarial evidence.** `docs/backtest/gate-stress-report.md` runs 12 scenarios against the real gate functions — deterministic, offline, byte-identical on every run. **11 are refusals the agent must make**, including the richest premium on the board (unlisted symbol), a vol spike that tempts it past the size cap, a -20% overnight gap, an overdrawn account, and the exact 2x-levered book that existed in production on 2026-09-02. The 12th confirms it still trades under normal conditions — a chain that refuses everything would prove nothing. Regenerate with `python scripts/gate_stress.py --write`.
